...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D, { k:getitem(u,k)+getitem(v,k) for k in { u_k for u_k,val in u.f.items() } | { v_k for v_k,val in v.f.items() } })
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([ getitem(u,k)*getitem(v,k) for k in { u_k for u_k,val in v.f.items() } & { v_k for v_k,val in v.f.items() } ])
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, { d:fval*alpha for d,fval in v.f.items() })
